function [u,v,x,T] = RunSimEta(dimension,q, beta, L, m,eta)
...
U0 = [uss*max(1+eta*randn(N,1),zeros(N,1));vss*max(1+eta*randn(N,1),zeros(N,1))];
...
odeOptions=odeset('JPattern',JacSparse,'RelTol',1e-11,'AbsTol',1e-11,'maxstep',0.1);
[T,U]=ode15s(F,[0,10, 50, 100],U0,odeOptions);

odeOptions=odeset('JPattern',JacSparse,'RelTol',1e-9,'AbsTol',1e-9);
[T,U]=ode15s(F,[0,1e1, 1e2, 1e3, 1e4, 1e5],U(end,:),odeOptions);
